...
export function HomeClient({ sharedLinkId, sharedSummary }: HomeClientProps) {
  ...
  const [isAutoGeneratingShare, setIsAutoGeneratingShare] = useState(false);
  const [isGeneratingShare, setIsGeneratingShare] = useState(false);
  const [hasAutoGeneratedShareAttempted, setHasAutoGeneratedShareAttempted] =
    useState(false);
  ...
  useEffect(() => {
    ...
    setIsAutoGeneratingShare(false);
    setIsGeneratingShare(false);
    setHasAutoGeneratedShareAttempted(false);
  }, [isSolved]);
  ...
  useEffect(() => {
    if (
      ...
      generatedShareId ||
      isAutoGeneratingShare ||
      hasAutoGeneratedShareAttempted
    ) {
      return;
    }

    setHasAutoGeneratedShareAttempted(true);
    let cancelled = false;
    const controller = new AbortController();
    let timeoutHandle: number | null = null;

    const autoGenerateShareId = async () => {
      setIsAutoGeneratingShare(true);
      try {
